...
df1 = pd.read_csv('Interaction_Files/Plant_Pollinator/Plant_Pollinator_Final.csv')
count = pd.read_csv('Interaction_Files/Plant_Pollinator/Plant_Count.csv')
...
G = nx.from_pandas_edgelist(df1, 'Source_Name', 'Target_Name', create_using=nx.DiGraph())
...
def pagerank(G):
    if len(G) <= 1:
        return 0
    return np.mean(list(dict(nx.pagerank(G)).values()))
...
def run_simulation(parameters, task_id):
    ...
    save_path = f'Results/Plant_Insect_Full/{selection}_{removal}/'
    filename = f"Perturbation_{selection}_{removal}_{str(task_id).zfill(4)}_plantpollinator.csv"
...
script_params = [
    {
        'selection_strategy': ['Random', 'Generalist', 'Specialist'],
        'removal_strategy': ['Random', 'Plant', 'Pollinator'],
        'ID': list(range(1000))  
    },
    {
